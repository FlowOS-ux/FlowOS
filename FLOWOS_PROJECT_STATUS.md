...
> **Last updated:** 2026-06 · **Status:** Active development (post-MVP-core)
...
```
> Steps in `[brackets]` are the **current onboarding gap** — see §12.
...
## 5. Current Frontend Status (~45%)

### Implemented screens (11)
| Area | Screen | Notes |
|---|---|---|
| Auth | **LoginScreen** | wired |
...
| Business | **BusinessesScreen** | dashboard / your businesses + per-queue actions + **＋ Business** FAB |
...
| Shared | **NotificationsScreen** | in-app feed (polls; not yet socket-driven) |
...
- **Business stack:** tabs (Dashboard, Notifications, Profile) + `CreateBusiness`, `QueueForm`, `QueueManager` (pushed).

### Completed flows (mobile)
- Customer: register → login → explore → join → **real-time live tracking**.
- Business: register → dashboard → **create business** → **create/edit/manage queues** → operate queue.

### Screens still pending
Splash (spinner only), Onboarding, **Forgot Password**, **Business Setup/Activate**, Appointments,
Saved Places, AI Assistant, Settings (editable), Reviews, dedicated Analytics, Staff Management,
Help & Support, Connection Error.
...
- **Frontend consumption is partial:** only Activity + Queue Manager are socket-driven; the
  **Notifications feed and business Dashboard still poll / refetch-on-focus** despite the backend
  emitting suitable events.
...
- **Notifications** — in-app `Notification` documents + feed API + `notification:new` emit + FCM
  dispatch service (coded). Mobile: `NotificationsScreen` (polled).
...
- `npm run smoke` → **19/19** (auth → business → queue → join → position/ETA → call→serve→complete
  → notification persisted → RBAC).
...
- **Business Setup + Activation UI** — DRAFT businesses can't be activated from the app, so they
  never appear in Explore. *(Design complete; implementation pending.)*
- **FCM background push** — notifications only arrive while the app is open.

### High priority
- **Forgot-Password screen** + real email provider config.
- **Notifications real-time consumption** (listen to `notification:new`) + dashboard real-time.
- **Block joining non-ACTIVE businesses** (backend guard).
...
| Area | % | Notes |
|---|---|---|
| Backend | **~85%** | 14 modules + ~55 endpoints; FCM/email stubbed |
| Frontend | **~45%** | 11 of ~27 screens; core + business queue mgmt done |
| Database | **~95%** | all collections + indexes |
| Real-time | **~85%** | backend done; client consumption partial |
| Notifications | **~40%** | in-app ✅, push ❌ |
| Testing | **~20%** | smoke + socket-smoke only; no unit/CI |
| **Overall** | **~58%** | strong backend, thinner frontend |
...
- FCM push + device-token registration on login.
- Forgot-Password screen + email provider.
- Cheap correctness wins: join-status guard; Notifications/dashboard socket consumption.
...
1. **Business Setup** — 7-day hours editor + edit existing setup (`GET`/`PATCH /businesses/:id`).
2. **Business Activation** — `status: 'ACTIVE'` toggle → makes the business discoverable in Explore
   (Explore filters on ACTIVE). *This is the single biggest gap to a working two-sided flow.*
3. **FCM Push Notifications** — wire `@react-native-firebase/messaging` + register device token on
   login (`POST /notifications/devices`); activate `FcmPushService` with a service account.
4. **Forgot Password UI** — screen for `forgot-password`/`reset-password` + real SMTP provider.
5. **Notifications real-time consumption** — `NotificationsScreen` (and dashboard) subscribe to the
   already-emitted `notification:new` / `dashboard:updated` events instead of polling.
...
- **Partial real-time on client** — Notifications feed + dashboard poll instead of consuming events.
...
- **Validation gaps** — business `category` is free-text (not checked against `categories`); join doesn't verify business is ACTIVE.
